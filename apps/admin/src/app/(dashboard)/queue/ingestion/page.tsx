...
const STATE_DESCRIPTIONS: Record<string, string> = {
  ...
  Failed: 'Errored — check the job details or worker logs',
}
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Ingestion Queue</h1>
      ...
      <div className="space-y-8">
        <section>
          {stats?.ingestion.error ? (
            <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-400">{stats.ingestion.error}</div>
          ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5 text-left text-xs text-zinc-500">
                  ...
                  <th className="px-5 py-3 font-medium">Finished / Error</th>
                </tr>
              </thead>
              <tbody>
                {jobs.length === 0 ? (
                  ...
                  <tr key={job.id} className="border-b border-white/5 last:border-0 hover:bg-white/2">
                    ...
                    <td className="px-5 py-3 text-xs">
                      {job.failedReason ? (
                        <span className="block max-w-xs truncate text-red-400" title={job.failedReason}>
                          ⚠ {job.failedReason}
                        </span>
