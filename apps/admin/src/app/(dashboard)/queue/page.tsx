...
const STATE_DESCRIPTIONS: Record<string, string> = {
  ...
  Delayed: 'Scheduled for a future time (normal for repeating ingestion jobs)',
...
export default function QueuePage() {
  ...
  const [agentJobs, setAgentJobs] = useState<QueueJob[]>([])
  const [ingestionJobs, setIngestionJobs] = useState<QueueJob[]>([])
  const [activeQueue, setActiveQueue] = useState<'agents' | 'ingestion'>('agents')
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const [statsData, agentJobsData, ingestionJobsData] = await Promise.all([
        api.getQueueStats(),
        api.getQueueJobs('agents', 20),
        api.getQueueJobs('ingestion', 20),
      ])
      setStats(statsData)
      setAgentJobs(agentJobsData.jobs)
      setIngestionJobs(ingestionJobsData.jobs)
  ...
  const displayJobs = activeQueue === 'agents' ? agentJobs : ingestionJobs
      ...
      <div className="space-y-8">
                ...
                return (
                  <div key={label} className="relative">
                    ...
                    <div className="absolute right-3 top-3">
                      <Tooltip text={STATE_DESCRIPTIONS[label] ?? ''} />
                    </div>
                  </div>
                )
              })}
        ...
        <section>
          ...
          {stats?.ingestion.error ? (
            ...
            <div className="grid gap-4 sm:grid-cols-5">
              {(['Waiting', 'Active', 'Delayed', 'Completed', 'Failed'] as const).map((label) => {
                const key = label.toLowerCase() as 'waiting' | 'active' | 'delayed' | 'completed' | 'failed'
                const val = stats?.ingestion[key] ?? '—'
                const variant = label === 'Active' && (stats?.ingestion.active ?? 0) > 0 ? 'success'
                  : label === 'Waiting' && (stats?.ingestion.waiting ?? 0) > 0 ? 'warning'
                  : label === 'Failed' && (stats?.ingestion.failed ?? 0) > 0 ? 'danger' : 'default'
                return (
                  <div key={label} className="relative">
                    <StatCard label={label} value={val} variant={variant} />
                    <div className="absolute right-3 top-3">
        ...
        <section>
          <div className="mb-3 flex items-center gap-3">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Recent Jobs</h2>
            <div className="flex gap-1">
              {(['agents', 'ingestion'] as const).map((q) => (
                <button
                  key={q}
                  onClick={() => setActiveQueue(q)}
                  className={`rounded px-2.5 py-1 text-xs font-medium transition capitalize ${activeQueue === q ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  {q}
                </button>
              ))}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {displayJobs.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-5 py-6 text-center text-xs text-zinc-600">
                      No recent jobs
                    </td>
                  </tr>
                ) : displayJobs.map((job) => (
